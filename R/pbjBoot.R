...
#' @param h HC3-related vector
#' @param sqrt_h square root of h
#'
#' @return Returns vector of test statistics computed from the bootstrapped sample.
#
pbjBoot = function(sqrtSigma, rboot=function(n){ (2*stats::rbinom(n, size=1, prob=0.5)-1)}, method=c('wild', 'permutation', 'nonparametric'), h, sqrt_h, robust=TRUE, transform=c('none', 't')){
  ...
  rdf = sqrtSigma$rdf
    if(robust){
        if(method=='wild'){#is.list(sqrtSigma)){ sqrtSigma should be a list here
          sqrtSigma$res = sweep(sqrtSigma$res, 1, rboot(n)/sqrt_h, '*')
          ...
          sqrtSigma$res = sweep(sqrtSigma$res[samp,], 1, sqrt_h[samp], '/')
...
# TODO: pass sqrtSigma components instead of entire list
pbjBootParallel <- function(sqrtSigma, rboot, method=c('wild', 'permutation', 'nonparametric'), h, sqrt_h, robust=TRUE, transform=c('none', 't')){
  ...
  if(robust) {
    if (method=='wild') {
      sqrtSigma$res = sweep(sqrtSigma$res, 1, rboot/sqrt_h, '*')
    } else if (method=='permutation') {
      sqrtSigma$res = sqrtSigma$res[sample(n),]
    } else if (method=='nonparametric') {
      samp = sample(n, replace=TRUE)
